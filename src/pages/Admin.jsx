...
import { Users, BookOpen, Target, BarChart2, Search, Shield, TrendingUp, Star } from 'lucide-react';
...
export default function Admin() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api.dashboard.get(), api.ranking.list(), api.content.list({}), api.missions.list()]).then(([dashboard, users, content, missions]) => {
      if (!active) return;
      setPayload({ metrics: dashboard.metrics, weeklyActivity: dashboard.weeklyActivity, regionActivity: dashboard.regionActivity, users, content, missions });
    });
    return () => { active = false; };
  }, []);
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between flex-wrap gap-4">
        <div><div className="flex items-center gap-2 mb-1"><Shield size={20} className="text-[#1A237E]" /><h2 className="text-2xl font-black text-[#0F172A]">Panel Admin</h2></div><p className="text-[#475569] text-sm">Control total del sistema.</p></div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-red-50 border border-red-200"><span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" /><span className="text-xs font-semibold text-red-700">Acceso Administrador</span></div>
      </motion.div>
          ...
          <div className="space-y-2">
            {filteredUsers.map((user, index) => (
              <motion.div key={user.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="card p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0"><p className="font-semibold text-sm text-[#0F172A] truncate">{user.name}</p><div className="flex items-center gap-2 mt-0.5"><span className="text-[10px] text-[#94A3B8]">{user.email}</span><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${roleBadge[user.role]}`}>{roleLabel[user.role]}</span></div></div>
