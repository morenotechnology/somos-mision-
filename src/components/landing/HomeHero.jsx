...
// Extend the existing navy/gold identity: the supplied invitation leads, uncropped.
// One clear invitation, one genuine live count; no invented proof or extra metrics.
      ...
      <div className="mission-home-layout">
        <div className="mission-home-copy">
          ...
          <div className="mission-home-actions">
            <button onClick={onRegister} className="mission-join">Unirme a la red <ArrowRight size={19} /></button>
            <button onClick={onLogin} className="mission-login">Ya soy parte · Iniciar sesión</button>
          </div>
        </div>
        <figure className="mission-film">
          <div className="mission-film-screen">
            ...
            {!playing && !videoError && <button className="mission-film-play" onClick={togglePlay} aria-label="Reproducir invitación"><Play size={28} fill="currentColor" /></button>}
          </div>
          <div className="mission-film-controls">
            <button onClick={togglePlay} disabled={videoError} aria-label={playing ? 'Pausar video' : 'Reproducir video'}>{playing ? <Pause size={18} /> : <Play size={18} />}</button>
            ...
            <span className="mission-film-time">{formatTime(time)} / {formatTime(duration)}</span>
            <button onClick={toggleSound} disabled={videoError} aria-label={muted ? 'Activar sonido' : 'Silenciar video'} aria-pressed={!muted}>
              {muted ? <VolumeX size={19} /> : <Volume2 size={19} />}<span>{muted ? 'Escuchar' : 'Sonido'}</span>
            </button>
          </div>
          <figcaption>{videoError ? <a href="/media/somos-mision-bienvenida.mp4">No se pudo reproducir. Abrir el video</a> : <>Una invitación para ti <span>Ps. Jairo Graffe · Director de Misiones Nacionales</span></>}</figcaption>
        </figure>
      </div>
      <div className="mission-community-row">
        ...
        <button onClick={onLearnMore} className="mission-more">Conoce la misión <span>Ver más <ArrowRight size={17} /></span></button>
      </div>
